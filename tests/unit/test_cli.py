...
def test_list_available_displays_basic():
    ...
    d = DisplayInfo(name=r"\\.\DISPLAY1", friendly_name="Fake",
                    ...
                    modes=[dm])
    with patch("winrandr.cli_handlers.list_displays", return_value=[d]):
...
def test_list_available_displays_with_providers():
    ...
    with patch("winrandr.cli_handlers.list_displays", return_value=[d]):
        with patch("winrandr.cli_handlers.list_providers", return_value=providers):
            result = _list_available_displays()
            assert "DISPLAY1" in result
...
def test_filter_valid_paths_out_of_range():
    ...
    paths = (DISPLAYCONFIG_PATH_INFO * 1)()
    modes = (DISPLAYCONFIG_MODE_INFO * 1)()
    paths[0].sourceInfo.modeInfoIdx = 5  # past mode_count
    paths[0].targetInfo.modeInfoIdx = 0
    modes[0].infoType = DISPLAYCONFIG_MODE_INFO_TYPE_TARGET
    assert filter_valid_paths(paths, 1, modes, 1) == []
...
def test_filter_valid_paths_wrong_type():
    """mode 类型不匹配（source→target、target→source）时过滤掉。"""
    paths = (DISPLAYCONFIG_PATH_INFO * 1)()
    modes = (DISPLAYCONFIG_MODE_INFO * 2)()
    paths[0].sourceInfo.modeInfoIdx = 0
    paths[0].targetInfo.modeInfoIdx = 1
    modes[0].infoType = DISPLAYCONFIG_MODE_INFO_TYPE_TARGET  # 应为 SOURCE
    modes[1].infoType = DISPLAYCONFIG_MODE_INFO_TYPE_SOURCE  # 应为 TARGET
...
def test_filter_valid_paths_mixed():
    ...
    modes[0].infoType = DISPLAYCONFIG_MODE_INFO_TYPE_SOURCE
    modes[1].infoType = DISPLAYCONFIG_MODE_INFO_TYPE_TARGET
    modes[2].infoType = DISPLAYCONFIG_MODE_INFO_TYPE_TARGET  # path[1] target
    modes[3].infoType = DISPLAYCONFIG_MODE_INFO_TYPE_TARGET
