...
- [PyPI 令牌](https://pypi.org/manage/account/token/) 已配置（`uv publish` 自动读取）
- 工作区干净，无未提交修改
- 当前在 `main` 分支
...
脚本依次执行以下 8 步，每步有确认提示：

| 步骤 | 操作 | 可跳过 |
|------|------|--------|
| 1 | 运行 `test.sh`（lint + pytest + 覆盖率） | 否 |
| 2 | 更新 `pyproject.toml` + `winrandr/__init__.py` 版本号 | 否 |
| 3 | 检查 `docs/CHANGELOG.md` 是否包含新版条目 | 否 |
| 4 | Nuitka 编译单文件 exe（约 2-5 分钟） | 否 |
| 5 | `git commit` + `git tag vX.Y.Z` | 否 |
| 6 | `git push origin main --tags` | 否 |
| 7 | `gh release create` 创建 GitHub Release 并上传 exe | 否 |
| 8 | `uv build` + `uv publish` 发布到 PyPI | 是（可在脚本外单独执行） |
